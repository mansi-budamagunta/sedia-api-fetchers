...
class ETLConfig:
    ...
    projects_staging_table: str = "raw_projects_staging"
    projects_published_table: str = "raw_projects"
...
def sqlite_safe_df(df: pd.DataFrame) -> pd.DataFrame:
    """Convert nested API values into JSON strings SQLite can store."""
    return df.map(
...
# class SQLiteSink:
#     """Write batches of raw API records to a fresh SQLite table."""
...
#         df = sqlite_safe_df(pd.DataFrame(records))
...
class DatabaseSink:
    """Write batches of raw API records to a fresh database table."""
    ...
    def start_fresh_load(self):
        """Discard the previous contents of this sink's target table."""
        with self.engine.begin() as connection:
            connection.execute(text(f'DROP TABLE IF EXISTS "{self.table_name}"'))
        self.rows_written = 0


    def count_rows(self, table_name: str | None = None) -> int:
        """Return the number of rows in a table, or zero when it does not exist."""
...
class ETLExtractor:
    ...
    def __init__(
        self,
        logger: ETLLogger,
        project_sink: DatabaseSink | None = None,
    ):
        self.logger = logger
        self.main_logger = logger.main_logger
        self.project_sink = project_sink
        self._import_fetchers()
            ...
            if endpoint == "projects":
                ...
                if rows_written > 0:
                    self.main_logger.info(
                        f"EXTRACT SUCCESS: streamed {rows_written:,} project records "
                ...
                self.main_logger.warning(
                    f"EXTRACT WARNING: No project data returned for {programme.name}"
                )
                ...
                return ExtractionResult(
                    ...
                    success=False,
                    record_count=0,
                    error_message="No project data returned",
                )
...
class ETLPipeline:
    ...
    def __init__(self, config: ETLConfig):
        ...
        # self.project_sink = DatabaseSink(
        #     config.db_url,
        #     "raw_projects",
        # )
        self.project_sink = DatabaseSink(
            config.db_url,
            config.projects_staging_table,
        )

        self.extractor = ETLExtractor(
            self.logger,
            project_sink=self.project_sink,
        )
...
def main():
    ...
    config = ETLConfig(
        ...
        db_url=os.environ.get(
                    "SEDIA_DB_URL",
                    f"sqlite:///{base_dir / 'data' / 'sedia.db'}",
                ),
    ...
    return (
        results["files_saved"] > 0
        or pipeline.project_sink.rows_written > 0
    )
